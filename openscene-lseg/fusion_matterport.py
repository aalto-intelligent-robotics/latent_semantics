...
def process_one_scene_mp(data_path: str, out_dir: str, num_rand_file_per_scene: int, depth_scale: float, keep_features_in_memory: bool, feat_dim: int, img_dim: tuple, visibility_threshold: float, cut_num_pixel_boundary: int, evaluator: LSeg_MultiEvalModule):
    '''Process one scene.'''
...
def main(args):
    ...
    # #int
    # cut_num_pixel_boundary = args.cut_num_pixel_boundary
    # Parallel(n_jobs=32, prefer="threads")(delayed(process_one_scene_mp)(data_path, out_dir, num_rand_file_per_scene, depth_scale, keep_features_in_memory, feat_dim, img_dim, visibility_threshold, cut_num_pixel_boundary, evaluator) for data_path in data_paths)
