...
from vlmaps.utils.index_utils import find_similar_category_id_static, get_segment_islands_pos, get_dynamic_obstacles_map_3d
...
class VLMap(Map):
    ...
    def load_map(self, data_dir: str, load_type: MapType = MapType.REGULAR) -> bool:
        map_prefix = self.map_config.map_prefix
        if (load_type == MapType.POSTPROCESSED):
            self.map_save_path = Path(data_dir) / "vlmap" / f"{map_prefix}-postprocessed.h5df"
        elif (load_type == MapType.INSTANCES):
            self.map_save_path = Path(data_dir) / "vlmap" / f"{map_prefix}-instances.h5df"
        elif (load_type == MapType.PREDICTED):
            self.map_save_path = Path(data_dir) / "vlmap" / f"{map_prefix}-predicted.h5df"
        elif (load_type == MapType.VLMAP_INSTANCES):
            self.map_save_path = Path(data_dir) / "vlmap" / f"{map_prefix}-vlmaps-instances.h5df"
        elif (load_type == MapType.OUR_INSTANCES):
            self.map_save_path = Path(data_dir) / "vlmap" / f"{map_prefix}-our-instances.h5df"
        elif (load_type == MapType.PREDICTED_POSTPROCESSED):
            self.map_save_path = Path(data_dir) / "vlmap" / f"{map_prefix}-predicted-postprocessed.h5df"
        else:
            self.map_save_path = Path(data_dir) / "vlmap" / f"{map_prefix}.h5df"
        return self.load_map_common(data_dir)
    ...
    def get_pos(self, name: str) -> Tuple[List[List[int]], List[List[float]], List[np.ndarray], Any]:
        ...
        contours, centers, bbox_list, _, mask = get_segment_islands_pos(foreground, 1)
        ...
        return contours, centers, bbox_list, mask
