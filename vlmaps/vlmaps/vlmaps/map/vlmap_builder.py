...
class VLMapBuilder:
    ...
    def create_mobile_base_map(self):
        ...
        self.map_save_path = self.map_save_dir / f"{self.map_config.map_prefix}.h5df"
        ...
        pbar = tqdm(zip(self.rgb_paths, self.depth_paths, self.base_poses, self.semantic_paths, self.region_paths), total=len(self.rgb_paths))
        for frame_i, (rgb_path, depth_path, base_posevec, semantic_path, region_path) in enumerate(pbar):
            ...
            for i, (p, p_local) in enumerate(zip(pc_global.T, pc.T)):
                ...
                if max_id >= grid_feat.shape[0]:
                    self._reserve_map_space(grid_feat, grid_pos, weight, grid_rgb, grid_semantic, grid_region, grid_instance)
                ...
                if not (px < 0 or py < 0 or px >= pix_feats.shape[3] or py >= pix_feats.shape[2]):
                    ...
                    if occupied_id == -1:
                        ...
                        semantics[occupied_id][region_v] += 1
            ...
            if frame_i % 100 == 99:
                ...
                self._save_3d_map(grid_feat, grid_pos, weight, grid_rgb,
                                  occupied_ids, mapped_iter_set, max_id, grid_semantic, grid_region, grid_instance)
        grid_semantic = self.getHighestValueFromDict(semantics, grid_semantic)
        grid_region = self.getHighestValueFromDict(regions, grid_region)
        self._save_3d_map(grid_feat, grid_pos, weight, grid_rgb,
                          occupied_ids, mapped_iter_set, max_id, grid_semantic, grid_region, grid_instance)
    ...
    def _init_map(self, camera_height: float, cs: float, gs: int, map_path: Path) -> Tuple:
        ...
        return vh, grid_feat, grid_pos, weight, occupied_ids, grid_rgb, mapped_iter_set, max_id, grid_semantic, grid_region, grid_instance
    ...
    def _reserve_map_space(
        self, grid_feat: np.ndarray, grid_pos: np.ndarray, weight: np.ndarray, grid_rgb: np.ndarray, grid_semantic: np.ndarray, grid_region: np.ndarray, grid_instance: np.ndarray
        ...
        grid_semantic = np.concatenate(
            [
                grid_semantic,
                np.zeros(
                    (grid_semantic.shape[0], grid_semantic.shape[1]), dtype=np.int32),
            ],
            axis=0,
        )
        grid_region = np.concatenate(
            [
                grid_region,
                np.zeros(
                    (grid_region.shape[0], grid_region.shape[1]), dtype=np.int32),
            ],
            axis=0,
        )
        grid_instance = np.concatenate(
            [
                grid_instance,
                np.zeros(
                    (grid_instance.shape[0], grid_instance.shape[1]), dtype=np.int32),
            ],
            axis=0,
        )
    ...
    def _save_3d_map(
        ...
        save_3d_map(self.map_save_path, grid_feat, grid_pos, weight, occupied_ids, list(
            mapped_iter_set), grid_rgb, grid_semantic, grid_region, grid_instance)
