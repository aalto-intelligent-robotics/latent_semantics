...
def load_semantic_npy(semantic_filepath: Union[Path, str]):
    print("path:", semantic_filepath)
...
def save_3d_map(
    ...
    grid_semantic: np.ndarray = None,
    grid_region: np.ndarray = None,
    ...
    with h5py.File(save_path, "w") as f:
        ...
        if grid_semantic is not None:
            f.create_dataset("grid_semantic", data=grid_semantic)
        if grid_region is not None:
            f.create_dataset("grid_region", data=grid_region)
        if grid_instance is not None:
            f.create_dataset("grid_instance", data=grid_instance)


def load_3d_map(map_path: str) -> Tuple[Set[int], np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    ...
    with h5py.File(map_path, "r") as f:
        ...
        if "grid_rgb" in f:
            grid_rgb = f["grid_rgb"][:]
        if "grid_semantic" in f:
            grid_semantic = f["grid_semantic"][:]
        if "grid_region" in f:
            grid_region = f["grid_region"][:]
        if "grid_instance" in f:
            grid_instance = f["grid_instance"][:]
        return mapped_iter_list, grid_feat, grid_pos, weight, occupied_ids, grid_rgb, grid_semantic, grid_region, grid_instance
